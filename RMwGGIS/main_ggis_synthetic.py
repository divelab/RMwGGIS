...
parser.add_argument('--is_dim', type=int, default=10, help='Number of average dimensions considered in importance sampling')
...
parser.add_argument('--activation', type=str, default='swish', help='Location for saving checkpoints')
parser.add_argument('--dropout', type=float, default=-1, help='Dropout rate')

### Training hyperparameter
parser.add_argument('--lr', type=float, default=1e-4, help='Learning rate')
parser.add_argument('--wd', type=float, default=0, help='Weight Decay')
parser.add_argument('--grad_clip', type=float, default=5, help='Clip gradient')
...
parser.add_argument('--unbiased', type=strtobool, default='false', help='Use unbiased estimator')
...
def train(model, db, device):
    ...
    if args.plot:
        ...
        print('Saving heat map at initialization')
    print('==========================================')
    ...
    for epoch in range(args.max_epochs):
        ...
        for i in pbar:
            ...
            energy_func_gradient = autograd.grad(energy_x.sum(), samples, create_graph=True)[0] ### [128, 32]
            weights = torch.exp(2*(2*samples-1)*energy_func_gradient) ### [128, 32]
            sampled_dims = torch.multinomial(weights, args.is_dim, replacement=False) ### [128, args.is_dim]

            
            ### Better implementation
            flip_samples = []
            for d in range(sampled_dims.shape[-1]):
                indx = sampled_dims[:,d]
                flip_samples_d = samples.clone()
                
                
                flip_samples_d[range(flip_samples_d.shape[0]),indx] = 1 - flip_samples_d[range(flip_samples_d.shape[0]),indx]
                
                flip_samples.append(flip_samples_d)
                
            flip_samples_cat = torch.cat(flip_samples, dim=0) ### [128*args.is_dim, 32]
            
            energy_xp = torch.reshape(model(flip_samples_cat), (samples.shape[0], -1)) ### [128, args.is_dim]
...
#             energy_xp = torch.cat(flip_energies, dim=1) ### [128*32, 32]
            ...
            likelihood_ratios = torch.exp(energy_x-energy_xp) ### [128, args.is_dim]
            
            if args.unbiased: ### Unbiased version
                probs = weights/torch.sum(weights, dim=-1, keepdim=True) ### [128, 32]
#                 prob = torch.stack([probs[i, sampled_dims[i]] for i in range(probs.shape[0])], dim=0)
                prob = torch.stack([probs[range(probs.shape[0]), sampled_dims[:,d]] for d in range(sampled_dims.shape[-1])], dim=-1)
                likelihood_ratios = likelihood_ratios/(prob * args.is_dim)**0.5  ### [128, args.is_dim]
       
            
            loss = torch.norm(likelihood_ratios, dim=-1)**2
...
#     device = torch.device('cpu')
    ...
    ### Initialize model
    model = EBM_MLP(args.discrete_dim, args.hidden_dim, bn=args.bn, activation=args.activation, dropout=args.dropout) #, std=args.std)
    print('==========================================')
    print(model)
    ...
    if args.plot_data:
        ...
        print('Visualization data samples')
        print('==========================================')
        sys.exit()
